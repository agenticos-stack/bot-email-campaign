<script>
  let { agent, onBack, onChange = () => {} } = $props();
...
<section class="chat" aria-label="Local agent conversation">
  ...
  <div class="messages" aria-live="polite">
    {#if messages.length === 0}
      <div class="empty"><strong>Work with the source</strong><p>Ask the agent to draft a campaign, shape an audience, or explain the current local state.</p></div>
    {/if}
