<script>
  ...
  // Each scenario is the set of fixture capability params the canvas stub reads.
  // `empty=1` + every door off is the first-run state that lands on setup.
  ...
  const adapter = createFixtureChatAdapter({
    context: { workspaceId: 'fixture-email-workspace', conversationId: 'fixture-email-chat' },
    handlers: { send: (request) => ({ ok: true, value:
      'This is a scripted response, not a live model. A real assistant would draft the campaign, propose subject lines, and stage the send for your approval — it never sends on its own. Your message has not been sent to any external service.' }) }
  });
  onDestroy(() => adapter.close());
  function changeScenario(value) { scenario = value; }
...
<header class="topbar">
  <div class="identity"><BrandMark /><span class="brand-name">Agentic<span class="brand-os">OS</span></span><span class="slash">/</span><strong>Email Campaign</strong><span class="edition" title={localRuntime ? 'Local SQLite with sample data. Drafts survive server restarts. No live AI or publishing.' : 'Synthetic data and scripted chat. No live AI or publishing.'}>DEV PREVIEW</span></div>
...
<main style="--bot-chat-width:392px">
  {#snippet chat()}
    <div class="conversation-heading"><strong>Email Campaign</strong><span>Conversation</span></div>
    <div class="transcript" bind:this={transcript} role="log" aria-label="Fixture conversation" aria-live="polite">
      <div class="assistant-intro"><span class="avatar" aria-hidden="true"><BrandMark /></span><div><strong>Email Campaign <small>Fixture assistant</small></strong><p>Start with the canvas. Open a draft, shape its audience and content, and bring the send to review.</p><p class="muted">You stay in control of the draft—and when it goes out.</p></div></div>
      {#if messages.length === 0}<div class="suggestions"><span class="eyebrow">TRY A STARTING POINT</span>{#each ['Draft a September members’ campaign', 'Propose a clearer subject line', 'Explain the review and send flow'] as prompt (prompt)}<button onclick={() => input = prompt}>{prompt}<span aria-hidden="true">↗</span></button>{/each}</div>{/if}
  ...
  {#snippet canvas()}
    ...
    <iframe src={frameUrl} title={localRuntime ? 'Email Campaign canvas — local SQLite sample' : 'Email Campaign canvas — synthetic fixture'}></iframe>
  {/snippet}
  <Shell {chat} {canvas} {chatOpen} chatSide="left" {mobilePane} canvasScroll="clip" chatLabel="Creative conversation" canvasLabel="Email Campaign canvas" />
</main>
{/if}
