<script>
  ...
  function declaredRequirement(key){
    return EMAIL_CAMPAIGN_DEFINITION.requirements.find(row=>row.requirementKey===key) ?? null;
  }
  ...
  // A chat turn that changed the draft bumps `revision`; the canvas re-reads
  // through its authoritative API on the `campaign` event its subscriber hears.
  $effect(()=>{revision;port?.postMessage({event:{type:'campaign'}});});
</script>
<!--
...
<iframe
  ...
  title="Email Campaign — local source"
  onload={connect}
></iframe>
{/key}
</div>
