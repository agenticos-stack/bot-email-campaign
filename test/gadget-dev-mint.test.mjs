...
test('refuses a key that is not shaped like one, before any request', () => {
  ...
  assert.throws(() => assertDeveloperKey('hunter2', 'EMAIL_CAMPAIGN_DEV_KEY'), /EMAIL_CAMPAIGN_DEV_KEY.*gadget_dev\.session/);
  assert.throws(() => assertDeveloperKey('ag_mcp_short', 'EMAIL_CAMPAIGN_DEV_KEY'), /gadget_dev\.session/);
  assert.doesNotThrow(() => assertDeveloperKey(KEY, 'EMAIL_CAMPAIGN_DEV_KEY'));
});
...
test('mints with the key as a bearer, and asks for this gadget', async () => {
  ...
  assert.deepEqual(JSON.parse(seen.init.body), { gadgetKey: 'email_campaign', title: 'Email Campaign' });
