...
test('local sessions resume one runtime, recheck identity and isolate organizations',async()=>{
  ...
  const session=createDevelopmentSessions({appKey:'test-app',origin:'http://email.localhost:18000',authenticate:async()=>identity,createRuntime:async key=>{
  ...
  const request=()=>new Request('http://email.localhost:18000/api/dev/rpc',{method:'POST',body:'{"method":"summary","args":[]}'});
...
test('agent calls receive the current request cookie, never the one captured at start',async()=>{
  ...
  const session=createDevelopmentSessions({appKey:'test-app',origin:'http://email.localhost:18000',
  ...
  const request=cookie=>new Request('http://email.localhost:18000/api/dev/agent',{method:'POST',headers:{cookie},body:'{"operation":"pending"}'});
...
test('agent calls receive the current gadget-dev token when no cookie is present',async()=>{
  ...
  const request=()=>new Request('http://email.localhost:18000/api/dev/agent',{method:'POST',body:'{"operation":"pending"}'});
...
test('start() and agent() report the live connection state, not a snapshot from creation',async()=>{
  ...
  const request=()=>new Request('http://email.localhost:18000/api/dev/agent',{method:'POST',headers:{cookie:'session=a'},body:'{"operation":"pending"}'});
...
test('door grants receive the current request cookie and require a started session',async()=>{
  ...
  const session=createDevelopmentSessions({appKey:'test-app',origin:'http://email.localhost:18000',
    ...
    createRuntime:async()=>({token:'host-only',agent:{info:{connected:true}},
      ...
      handle:async()=>Response.json({ok:true}),dispose:async()=>{}})});
  const request=cookie=>new Request('http://email.localhost:18000/api/dev/grant',{method:'POST',headers:{cookie}});
  const input={requirementKey:'favcrm_connector',persistToAgent:false};
...
test('connection calls reach the started runtime only',async()=>{
  ...
  const session=createDevelopmentSessions({appKey:'test-app',origin:'http://email.localhost:18000',
    ...
    createRuntime:async()=>({token:'host-only',agent:{info:{connected:true}},
      ...
      handle:async()=>Response.json({ok:true}),dispose:async()=>{}})});
  const request=()=>new Request('http://email.localhost:18000/api/dev/connections',{method:'POST',headers:{cookie:'session=a'}});
