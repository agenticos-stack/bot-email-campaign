...
test('connected BFF forwards only the owner answer to a door grant',async()=>{
  const inputs=[];
  const handle=createConnectedApi({apiOrigin:'http://127.0.0.1:8789',frontendOrigin,development:{grant:async(_request,value)=>{inputs.push(value);return {requirementKey:'workspace',persistedToAgent:false};}},fetcher:()=>assert.fail('grant must go through the development host')});
  ...
  assert.equal((await handle(request({requirementKey:'workspace'}))).status,400);
  assert.equal((await handle(request({requirementKey:'workspace',persistToAgent:'no'}))).status,400);
  assert.equal((await handle(request({requirementKey:'workspace',persistToAgent:false,resolvedId:'x'}))).status,400);
  assert.equal(inputs.length,0);
  const response=await handle(request({requirementKey:'workspace',persistToAgent:false}));
  assert.equal(response.status,200);
  assert.deepEqual(await response.json(),{data:{requirementKey:'workspace',persistedToAgent:false}});
  assert.deepEqual(inputs,[{requirementKey:'workspace',persistToAgent:false}]);
  ...
  const failed=await refused(request({requirementKey:'favcrm_connector',persistToAgent:false}));
  ...
  const unknown=await lost(request({requirementKey:'workspace',persistToAgent:false}));
...
test('connected BFF admits only a connection listing or one account choice',async()=>{
  const inputs=[];
  const handle=createConnectedApi({apiOrigin:'http://127.0.0.1:8789',frontendOrigin,development:{connections:async(_request,value)=>{inputs.push(value);return value.operation==='list'?{families:[]}:{granted:{requirementKey:'favcrm_connector:favcrm_main'}};}},fetcher:()=>assert.fail('connections go through the development host')});
  ...
  for(const bad of [{},{operation:'revoke'},{operation:'list',requirementKey:'favcrm_connector'},{operation:'grant',requirementKey:'favcrm_connector'},{operation:'grant',requirementKey:'favcrm_connector',resolvedId:7},{operation:'grant',requirementKey:'favcrm_connector',resolvedId:'crb_1',resolvedLabel:'x'}])
  ...
  assert.equal((await handle(request({operation:'grant',requirementKey:'favcrm_connector',resolvedId:'crb_1'}))).status,200);
  assert.deepEqual(inputs,[{operation:'list'},{operation:'grant',requirementKey:'favcrm_connector',resolvedId:'crb_1'}]);
});
