// A hand-rolled DOM, just large enough to smoke-load the Email
// Campaign client's bundled client.js in plain node.
//
...
export class DocumentShim {
  ...
  /** Document-level listeners — the client delegates clicks/inputs to `document`. */
  addEventListener(type: string, handler: Listener): void {
    const list = this.listeners.get(type) ?? [];
    list.push(handler);
    this.listeners.set(type, list);
  ...
  async dispatchEvent(event: Record<string, unknown>): Promise<void> {
    const handlers = this.listeners.get(String(event.type)) ?? [];
    await Promise.all(handlers.map((handler) => handler({ ...event, currentTarget: this })));
  }
...
export function installMinimalDom(): { document: DocumentShim; gadgetRoot: ElementShim } {
  ...
  // `window` is globalThis here; the client listens for `message` (the grant
  // postMessage protocol) and `online` on it. A node global has no event
  // surface, so provide the minimal listener map those calls need.
