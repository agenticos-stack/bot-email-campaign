 ...
 * browser on `email.localhost` sees neither it nor the token it mints.
 */
...
export function assertDeveloperKey(key) {
  if (!KEY_PATTERN.test(key ?? '')) {
    throw new Error('EMAIL_CAMPAIGN_DEV_KEY must be a personal access token (ag_mcp_…) carrying the gadget_dev.session scope.');
  }
}
