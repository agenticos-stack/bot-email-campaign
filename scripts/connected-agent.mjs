...
export async function createConnectedAgent({
  ...
  if (remote) {
    ...
    assertGadgetDevWorkspaceId(boundWorkspaceId, 'EMAIL_CAMPAIGN_DEV_WORKSPACE_ID');
  } else {
    assertLocalApiOrigin(apiOrigin);
    assertLocalFrontendOrigin(frontendOrigin, ['email.localhost']);
  }
  /*
  ...
  async function maybeRenew() {
    ...
    try {
      ...
      // every subsequent read starts asking the owner for approval.
...
export function emailMethodNames() {
  return [
