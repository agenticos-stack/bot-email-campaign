...
const LOCAL_FRONTEND_HOSTS = new Set(['email.localhost', '127.0.0.1', 'localhost']);
...
export function assertGadgetDevWorkspaceId(workspaceId) {
  if (typeof workspaceId !== 'string' || !/^chat_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(workspaceId))
    throw new Error('EMAIL_CAMPAIGN_DEV_WORKSPACE_ID must be the chat_ workspace id returned when the development token was minted.');
}
